...
### Manche v0.10 -- selection par port
...
la règle de méthode publiée depuis v0.10 est une sélection par port :

- départ du catalogue harmonique étendu porté dans `amar-core` ;
...
- conservation finale des seuls constituants statistiquement significatifs
  (`SNR >= 2`) après un ajustement moindres carrés sur la calibration ;
- jugement uniquement sur une fenêtre de validation réservée, jamais sur le
  RMS de calibration.

Cette sélection ne change pas le moteur : les packs restent en
`station_harmonics_v0`, mêmes conventions temporelles et mêmes corrections
nodales. Elle choisit seulement le jeu de constituants. Si elle devient un
modèle publié pour un port déjà packagé, ou si elle qualifie un nouveau port,
le résultat doit être une nouvelle `data_version`, avec constituants, fenêtre
et benchmark documentés. Les anciens packs et benchmarks restent byte-stables.

Le témoin anti-surajustement est Concarneau : la sélection par port y reproduit
le comportement du jeu courant sans gain matériel, donc elle n'est pas publiée
pour les ports propres déjà inclus.
